...
onSnapshot(query(collection(db,"sales"), orderBy("created","desc")), snap=>{
 ...
 rows.forEach((r,i)=>{
  ...
  tr.innerHTML=`<td>${i+1}</td><td>${r.agent}</td><td>${r.daily}</td><td>${r.weekly}</td><td>${r.monthly}</td><td>${r.avgUp}</td><td>${r.avgMo}</td><td>${r.yearly}</td>`;
  tbody.appendChild(tr);
 });

 recentDiv.innerHTML=sales.slice(0,10).map(s=>`<div>${s.agent}: €${s.upfront}+€${s.monitoring} — ${new Date(s.time).toLocaleString()}</div>`).join("");
